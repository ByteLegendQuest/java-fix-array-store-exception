...
`git clone https://github.com/ByteLegendQuest/java-fix-array-store-exception`

A project directory named `java-fix-array-store-exception` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-fix-array-store-exception` directory which is created just now.
